...
*/
    ...
    cd(date: any) {
        if(date){
            let dat = moment(date).format('DD/MM/YYYY HH:mm:ss');
            return (dat == 'Invalid date') ? 'ไม่มีข้อมูล' : dat
        }else{
            return '-'
        }
    ...
    dateTH(date?: moment.Moment) {
        ...
        return day + ' ' + moment(date).add(543, 'year').format('DD/MM/YYYY')
    } 
    dateTTH(date?: moment.Moment) {
        let dd = moment.locale('th')
        let day = moment(date).format('dddd')
        return day + ' ' + moment(date).add(543, 'year').format('DD/MM/YYYY HH:mm:ss')
    } 
    comma(x: number) {
        try {
            let y = Number(x)
