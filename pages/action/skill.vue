     ...
     <v-dialog v-model="dialog" scrollable persistent max-width="500px" transition="dialog-transition">
         <v-card>
             <v-card-title primary-title>
                 จัดการความสามารถพนักงาน <v-spacer></v-spacer>
...
export default {
    ...
    methods: {
        async run() {
            this.response = false;
            this.items = await this.$core.getHttp(`/api/skill`);
        ...
        async update() {
            ...
            if (check && this.$refs.form.validate()) {
                let data = await this.$core.putHttp('/api/skill/' + this.form.id , this.form)
                if (data.id) {
        ...
        async remove(item) {
            ...
            if (check) {
                let data = await this.$core.deleteHttp('/api/skill/' + item.id )
                if (data) {
        ...
        async store() {
            if (this.$refs.form.validate()) {
                let data = await this.$core.postHttp(`/api/skill`, this.form)
                if (data.id) {
                    await this.$web.alertAuto(`บันทึกข้อมูลสำเร็จ`);
                    await this.run();
                } else {
