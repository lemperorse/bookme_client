     ...
     <v-dialog v-model="dialog" scrollable persistent max-width="500px" transition="dialog-transition">
         <v-card>
             ...
             <v-card-text>
                 <div class="mt-2">
                     <v-form ref="form">
                         <v-text-field outlined dense type="text" label="ชื่อกลุ่มขอบเขตงาน" v-model="form.name" :rules="[]"></v-text-field>
                         <v-autocomplete label="ขอบเขตบริการ" outlined dense chips small-chips v-model="form.works" multiple :items="works" item-text="name" item-value="name"></v-autocomplete> 
                         <div class="flex">
                             <v-spacer></v-spacer>
...
export default {
    ...
    methods: {
        async run() {
            this.response = false;
            this.works = await this.$core.getHttp(`/api/work`);
            this.items = await this.$core.getHttp(`/api/workgroup`);
        ...
        async openUpdateDialog(item) {
            this.form = item
            this.form.works = (this.form.works).split(',')
            this.dialog = true
        },
        async update() {
            ...
            if (check && this.$refs.form.validate()) {
                this.form.works = (this.form.works).toString()
                let data = await this.$core.putHttp('/api/workgroup/' + this.form.id , this.form)
                if (data.id) {
        ...
        async remove(item) {
            ...
            if (check) {
                let data = await this.$core.deleteHttp('/api/workgroup/' + item.id )
                if (data) {
        ...
        async store() {
            if (this.$refs.form.validate()) {
                this.form.works = (this.form.works).toString()
                let data = await this.$core.postHttp(`/api/workgroup`, this.form)
                if (data.id) {
                    await this.$web.alertAuto(`บันทึกข้อมูลสำเร็จ`);
                    await this.run();
                } else {
