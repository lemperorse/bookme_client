    ...
    <div>
        <v-expansion-panels flat  v-model="panel" multiple v-if="response">
            ...
            <v-expansion-panel  class="border-2 border-gray-300"  >
                <v-expansion-panel-header> <span class="font-semibold text-xl">ขอบเขตบริการ</span>  </v-expansion-panel-header>
                <v-expansion-panel-content>
                    <v-form ref="step2">
                                <div class="flex flex-col md:flex-row">
                                    <div class="w-full md:w-1/2 p-2">
                                        <h2 class="font-semibold text-xl">เขตบริการ</h2>
                                        <v-autocomplete persistent-hint :rules="[$v.req]" class="mt-4" @change="changeWorkGroup()" :items="[{'name':'ไม่มีกลุ่มเขต','works':[]},...env.workgroups]" item-text="name" item-value="name" v-model="form.work_group" dense outlined label="แผนปฏิบัติการทำความสะอาด">
                                        </v-autocomplete>

                                        <v-checkbox @change="changeWorkGroupAll()" v-model="form.work_content_all" dense label="เลือกทั้งหมด"></v-checkbox>

                                        <v-autocomplete chips :items="env.manageworks" item-text="name" item-value="name" :rules="[$v.req, $v.reqSelect ]" multiple v-model="form.work_content" outlined label="เลือกเขตบริการ">
                                        </v-autocomplete>
                                    ...
                                    <div class="w-full md:w-1/2 p-2">
                                        ...
                                        <v-checkbox @change="changeEquipmentAll()" v-model="form.equipment_all" dense label="เลือกทั้งหมด"></v-checkbox>
                                        <v-autocomplete v-model="form.equipment" multiple :items="env.equipments" item-text="name" item-value="name" chips outlined label="เลือกงานพิเศษ"></v-autocomplete>
                            ...
                            <v-toolbar dense flat color="transparent">
                                    <v-spacer></v-spacer>
                                    <v-btn depressed @click="($refs.step2.validate())?submitForm():''" color="warning">แก้ไขข้อมูล</v-btn>
                                </v-toolbar>
                 </v-expansion-panel-content>
            </v-expansion-panel>
            <v-expansion-panel  class="border-2 border-gray-300"  >
                <v-expansion-panel-header ><span class="font-semibold text-xl">รายละเอียดการทำงาน</span>  </v-expansion-panel-header>
                <v-expansion-panel-content>
                    <v-form ref="step3">
                                ...
                                <div class="flex flex-col md:flex-row">
                                    ...
                                    <div class="w-full md:w-1/4 p-1">
                                        <v-text-field type="number" v-model="form.period_count" dense outlined label="จำนวนวัน"></v-text-field>
                                    </div>
                                    <div class="w-full md:w-1/4 p-1">
                                        <v-combobox dense outlined :items="['9:00 - 17:00','22:00 - จนแล้วเสร็จ']" v-model="form.period_date" label="ระยะเวลานัดหมาย"></v-combobox>

                                    </div>
                                    <div class="w-full md:w-1/4 p-1">
                                        <v-text-field persistent-hint :hint="getHintPeople()" v-model="form.number_people" dense outlined label="จำนวนคนที่ต้องการ">
                                        </v-text-field>
                                    </div>
                                    <div class="w-full md:w-1/4 p-1">
                                        <v-text-field persistent-hint :hint="getHintArea()" v-model="form.area_size" dense outlined label="ขนาดพื้นที่"></v-text-field>
                                    </div>
                                    <div class="w-full md:w-1/4 p-1">
                                        <v-select :rules=[$v.req] v-model="form.area_meter" :items="[`ตารางเมตร`,`ตารางวา`]" dense outlined label="หน่วย"></v-select>
                                    </div>
                                </div>
                            ...
                            <v-toolbar dense flat color="transparent">
                                    <v-spacer></v-spacer>
                                    <v-btn depressed @click="($refs.step3.validate())?submitForm():''" color="warning">แก้ไขข้อมูล</v-btn>
                                </v-toolbar>
                 </v-expansion-panel-content>
            </v-expansion-panel>
            <v-expansion-panel  class="border-2 border-gray-300"  >
                <v-expansion-panel-header > <div class="flex ">
                        ...
                        <span class="ml-2 font-semibold text-xl">ข้อความสำหรับส่งไลน์</span>
